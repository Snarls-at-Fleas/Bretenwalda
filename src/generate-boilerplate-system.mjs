...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'bretenwalda', 'Bretenwalda', and 'BOILERPLATE'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.bretenwalda/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.bretenwalda/g),
   ...
   * Rename files.
   *
   * Rename files that had bretenwalda in their name, such as
   * css/bretenwalda.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*bretenwalda*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('bretenwalda', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-bretenwalda-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Bretenwalda's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace bretenwalda name mentions in files.
      generator.replaceFileContents();
      // Rename files that had bretenwalda in their name.
